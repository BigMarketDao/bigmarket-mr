...
router.get('/token/verify', async (req, res) => {
	...
	console.log('/token/verify: g_javascript_origins: ' + getConfig().g_javascript_origins);
	const accessToken = tokenResponse.data.access_token;
	const redirectUrl = `${getConfig().g_javascript_origins}/auth/callback?token=${encodeURIComponent(accessToken)}`;
