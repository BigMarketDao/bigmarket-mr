...
router.get(
  "/extensions/is-core-team-member/:emergencyExecuteContractId/:stacksAddress",
  async (req, res, next) => {
    try {
      const result = isExecutiveTeamMember(
        req.params.emergencyExecuteContractId,
...
router.get(
  "/extensions/is-core-team-member/:stacksAddress",
  async (req, res, next) => {
    try {
      const emergencyExecuteContractId =
        getDaoConfig().VITE_DOA_DEPLOYER +
        "." +
        getDaoConfig().VITE_DOA_EMERGENCY_EXECUTE_EXTENSION;
      const result = await isExecutiveTeamMember(
...
router.get(
  "/extensions/get-governance-data/:daoContractId/:stacksAddress",
  async (req, res, next) => {
    try {
      const result = getGovernanceData(
        req.params.daoContractId.split(".")[0],
        req.params.stacksAddress
      );
      res.send(result);
    } catch (error) {
...
router.get("/extensions/is-extension/:extensionCid", async (req, res, next) => {
  try {
    ...
    const result = await isExtension(
      extensionEvent.daoContract.split(".")[0],
      extensionEvent.daoContract.split(".")[1],
      req.params.extensionCid
    );
...
router.get(
  "/extensions/get-signals/:daoContractId/:stacksAddress",
  async (req, res, next) => {
    try {
      const result = getGovernanceData(
        req.params.daoContractId.split(".")[0],
