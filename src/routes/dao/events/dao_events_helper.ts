...
async function findBaseDaoEventByContractAndIndex(daoContract: string, event_index: number, txId: string): Promise<any> {
...
export async function fetchBaseDaoEvents(): Promise<Array<DaoEventEnableExtension>> {
...
export async function fetchExtensions(daoContract: string): Promise<any> {
	const result = await daoEventCollection.find({ daoContract, event: 'extension' }).toArray();
	return result;
}

export async function fetchExtensionEvent(extension: string): Promise<DaoEventEnableExtension> {
	const result = await daoEventCollection.findOne({
		extension,
		event: 'extension'
	});
