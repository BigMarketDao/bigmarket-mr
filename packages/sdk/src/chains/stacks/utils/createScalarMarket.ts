...
import { dataHashMarketSip18, marketDataToTupleCV } from "./signatures.js";
import { estimateBitcoinBlockTime, formatFiat } from "@bigmarket/bm-utilities";
...
const bitcoinLogo =
  "https://media.istockphoto.com/id/1139020309/vector/bitcoin-internet-money-icon-vector.jpg?s=612x612&w=0&k=20&c=vcRUEDzhndMOctdM7PN1qmipo5rY_aOByWFW0IkW8bs=";
const stacksLogo =
...
export function generateScalarMarketData(
  address: string,
  deployer: string,
  ...
  const endCooling = currentHeight + duration + coolDown;
  const ends = estimateBitcoinBlockTime(endCooling, currentHeight);
  const meta = getMetaData(currency, rates, chain, endCooling, ends);
  const market = convertMarketToLocalFormat(
    address,
    deployer,
    treasuryContract,
    network,
  ...
  market.criterionDays = { duration, coolDown, startHeight: currentHeight };
  market.criterionSources = meta.criterionSources;
  return market;
}

function generateOutcomeCategories(
  price: number,
  variance: number,
): Array<ScalarMarketDataItem> {
  const step = price * variance; // 5% increment
  const categories: Array<{ min: number; max: number }> = [];
  for (let i = -2; i <= 2; i++) {
...
function getMetaData(
  ...
  let coin = "BTC";
  let price = formatFiat(currency, btcPrice!);
  ...
  if (chain === STXUSD) {
    coin = "STX";
    price = formatFiat(currency, stxPrice);
    cats = generateOutcomeCategories(stxPrice, 0.06);
    priceFeedId = STXUSD;
    logo = stacksLogo;
  } else if (chain === SOLUSD) {
    coin = "SOL";
    price = formatFiat(currency, solPrice);
    cats = generateOutcomeCategories(solPrice, 0.04);
    priceFeedId = SOLUSD;
    logo = solanaLogo;
  } else if (chain === ETHUSD) {
    coin = "ETH";
    price = formatFiat(currency, ethPrice);
    cats = generateOutcomeCategories(ethPrice, 0.04);
    priceFeedId = ETHUSD;
    logo = ethereumLogo;
  } else if (chain === SUIUSD) {
    coin = "SUI";
    price = formatFiat(currency, suiPrice);
    cats = generateOutcomeCategories(suiPrice, 0.04);
    priceFeedId = SUIUSD;
    logo = suiLogo;
  } else if (chain === TONUSD) {
    coin = "TON";
    price = formatFiat(currency, tonPrice);
...
function convertMarketToLocalFormat(
  address: string,
  deployer: string,
  treasuryContract: string,
  network: string,
  ...
  const marketMeta = {
    ...
    proposer: address,
    treasury: `${deployer}.${treasuryContract}`,
    token: token.token,
    merkelRoot: "",
    contractIds: [],
    social: {
    ...
    featured: true,
    processed: false,
    liquidity: token.minLiquidity || 0,
    criterionDays: {
      duration: DURATION,
      coolDown: COOL_DOWN,
      startHeight: current,
  ...
  const tupleMessage = marketDataToTupleCV(
    marketMeta.name,
    marketMeta.category,
    marketMeta.createdAt,
    marketMeta.proposer,
    marketMeta.token,
  );
  const domain: Domain = {
  ...
  const dataHash = dataHashMarketSip18(domain, tupleMessage);
