<script lang="ts">
	...
	import { appConfigStore, requireAppConfig, requireDaoConfig, daoConfigStore, allowedTokenStore } from '@bigmarket/bm-common';
	import { getStxAddress } from '@bigmarket/bm-common';
  import { fullBalanceInSip10Token, getGovernanceToken } from '@bigmarket/bm-utilities';
  import type { Sip10Data } from '@bigmarket/bm-types';
	const daoConfig = $derived(requireDaoConfig($daoConfigStore));
	const appConfig = $derived(requireAppConfig($appConfigStore));
	let sip10Data: Sip10Data = $derived(getGovernanceToken(daoConfig.VITE_DAO_DEPLOYER, daoConfig.VITE_DAO_GOVERNANCE_TOKEN, $allowedTokenStore));
	let totalBalanceUstx: number = 0;

	let amountStx: number = $state(0);
	...
	onMount(async () => {
		...
		totalBalanceUstx = await fullBalanceInSip10Token(appConfig.VITE_STACKS_API, getStxAddress(), `${daoConfig.VITE_DAO_DEPLOYER}.${daoConfig.VITE_DAO_GOVERNANCE_TOKEN}`);
	});
</script>
