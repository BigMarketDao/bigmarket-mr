<script lang="ts">
  ...
  const labelClass =
    "text-xs font-medium uppercase tracking-wide text-foreground/50";
    ...
    market?.marketData?.stakes
      ? (
          (Number(market.marketData.stakes[0]) /
            market.marketData.stakes.reduce(
  ...
  const totalStakeMicro = $derived(
    market.marketData.stakeTokens?.reduce(
      (sum: number, stake: number) => sum + Number(stake),
      0,
    ) || 0,
  );
  ...
  const totalStakedDisplay = $derived(
    Number(
      (totalStakeMicro / Math.pow(10, sip10Data.decimals ?? 6)).toFixed(2),
    ),
  );
  ...
  const totalStakedFiatLine = $derived.by(() => {
    try {
      ...
      const fiatAmount = Number(
        toFiat(rate, totalStakeMicro, sip10Data, undefined, 2),
      );
  ...
  onMount(async () => {
    ...
    try {
      ...
      preselectIndex =
        p !== null && !Number.isNaN(Number(p)) ? Number(p) : null;
    } catch {}
...
<div
  class="mb-8 grid grid-cols-2 items-stretch gap-4 lg:grid-cols-4"
  data-market-stats
>
  <!-- Card 1 — Crowd says -->
  ...
  <div class={cardClass} data-volume-display>
    ...
    {#if totalStakedFiatLine}
      <p class="text-sm font-normal text-foreground/50 tabular-nums">
        {totalStakedFiatLine}
      </p>
    {/if}
    ...
    <div class="mt-1 text-sm">
      ...
      {:else if market.marketData.resolutionState === ResolutionState.RESOLUTION_OPEN}
        ...
        {#if market.marketType === 2}
          <span class="{subClass} block"
            >Feed Id: {market.marketData.priceOutcome}</span
          >
        {/if}
      {/if}
    </div>
