<script lang="ts">
  import type { Currency, MarketCategory, Payout, PredictionMarketCreateEvent, Sip10Data } from "@bigmarket/bm-types";
...
const isCategoricalMarket = $derived(
	market?.marketType === 1 && (market?.marketData?.categories?.length ?? 0) > 2
);

const isScalarMarket = $derived(market?.marketType === 2);
  onMount(() => {
    inited = true;
  });
</script>
          ...
          {:else}
            ...
            <div class="flex flex-col">
              {#each market.marketData.categories as category, i}
                <div
                  class="flex items-center justify-between rounded px-1 py-0.5 text-[11px] hover:bg-gray-50 dark:hover:bg-gray-800"
                >
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a
                      ...
                      class="rounded bg-gray-100 py-0.5 text-[8px] font-medium text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                      >Trade</a
                    >
                  </div>
                </div>
      ...
      <div class="mt-3 flex justify-between text-[10px]">
        <div>
          ...
          {:else if disputeRunning}
            ...
            />
          {:else}
            {getOutcomeMessageOneWord(currentBurnHeight, disputeWindowLength, marketVotingDuration, market)}
          {/if}
        </div>
        <div class="">
