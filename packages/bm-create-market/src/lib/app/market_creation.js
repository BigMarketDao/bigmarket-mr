...
import { getStxAddress, showTxModal, watchTransaction, } from "@bigmarket/bm-common";
import { CREATE_MARKET_TIER, getBnsNameFromAddress, getGateKeeper, getMarketToken, getTierBalance, MESSAGE_BOARD_ID, postCreatePollMessage, } from "@bigmarket/bm-utilities";
...
export async function getSignature(appConfig, daoConfig, template, daoOverview) {
    ...
    if (!daoOverview.contractData) {
        ...
        throw new Error("Application data not loaded. Please refresh the page and try again.");
    }
    // Sync template data to examplePoll before validation
    const profanity = new Profanity();
    template.description = profanity.censor(template.description);
    template.criterionSources.criteria = profanity.censor(template.criterionSources.criteria);
    ...
    if (template.criterionDays) {
        ...
        if (!template.criterionSources.sources ||
    ...
    console.log("forumContent: appConfig.VITE_FORUM_API: " + appConfig.VITE_FORUM_API);
    console.log("forumContent: forumContent.messageId: " + forumContent.messageId);
    ...
    console.log("forumContent: dataHash: " + dataHash);
    let valid = stacks.verifyForumSignature(domain, forumContent, publicKey, signature);
    try {
        const threads = (await createThread(appConfig.VITE_FORUM_API, forumContent.messageId, {
            forumContent,
            auth: {
    ...
    const result = await postCreatePollMessage(appConfig.VITE_BIGMARKET_API, poll);
...
export async function confirmPoll(appConfig, daoConfig, dataHash, template, daoOverview, tokenEvent) {
    if (!daoOverview.contractData) {
        throw new Error("Application data not loaded. Please refresh the page and try again.");
    }
    ...
    if (template.marketType === 1) {
        ...
        if (!template.marketTypeDataScalar ||
            ...
            throw new Error("Contiguous values only - the min must be the max of the previous");
        }
        contractName = daoConfig.VITE_DAO_MARKET_SCALAR;
    }
    const sender = getStxAddress();
    const tierBalance = await getTierBalance(appConfig.VITE_BIGMARKET_API, CREATE_MARKET_TIER, sender);
    const creationGated = await stacks.canCreateMarket(appConfig.VITE_BIGMARKET_API, getStxAddress());
    const gateKeeper = await getGateKeeper(appConfig.VITE_BIGMARKET_API);
    const marketTypeData = template.marketType === 2
        ? template.priceFeedId
        : template.marketTypeDataCategorical;
    const response = await stacks
        .createMarketsClient(daoConfig)
        .createMarket(contractName, gateKeeper, creationGated, tokenEvent, template.treasury, sender, template.marketFee, dataHash, template.liquidity, marketTypeData, template.criterionDays.duration, template.criterionDays.coolDown, tierBalance || 0);
    if (response.success && response?.txid) {
        showTxModal(response.txid);
        watchTransaction(appConfig.VITE_BIGMARKET_API, appConfig.VITE_STACKS_API, `${daoConfig.VITE_DAO_DEPLOYER}.${contractName}`, response.txid);
    }
    else {
...
//# sourceMappingURL=market_creation.js.map