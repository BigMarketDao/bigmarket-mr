...
export type ValidationResult = {
  isValid: boolean;
  errors: Record<string, string>; // key = field, value = error message
};
...
export function validatePoll(
  ...
  if (!feeResult.isValid) errors.marketFee = feeResult.message;

  // Description
  const descResult = validateDescription(poll.description);
  if (!descResult.isValid) errors.description = descResult.message;

  // Logo
  const logoResult = validateLogo(poll.logo);
  if (!logoResult.isValid) errors.logo = logoResult.message;

  // Market Type
  if (poll.marketType === undefined) {
    errors.marketType = "Market type is required";
  } else if (
    (poll.marketType === 0 || poll.marketType === 1) &&
  ...
  // Criteria text
  if (
    !poll.criterionSources?.criteria ||
  ...
  if (!treasuryResult.isValid) errors.treasury = treasuryResult.message;

  // Social links
  const twitterResult = validateTwitter(
    poll.social?.twitter?.projectHandle ?? "",
  );
  if (!twitterResult.isValid) errors.twitter = twitterResult.message;

  const discordResult = validateDiscord(poll.social?.discord?.serverId ?? "");
  if (!discordResult.isValid) errors.discord = discordResult.message;

  const websiteResult = validateWebsite(poll.social?.website?.url ?? "");
  if (!websiteResult.isValid) errors.website = websiteResult.message;
...
export function isValidURL(string: string): boolean {
  try {
    ...
    if (url.protocol !== "https:" && url.protocol !== "data:") return false;
    ...
    if (url.username || url.password) return false; // No auth in URLs
    if (url.href.length > 2000) return false; // Reasonable length limit
...
export function isValidStacksAddress(address: string): boolean {
  ...
  const isValidBody =
    /^[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz0]+$/.test(
      addressBody,
    );
...
export function validateMarketFee(fee: number, marketFeeBipsMax: number) {
...
export function validateTreasury(treasury: string) {
  ...
  try {
    if (trimmed.includes(".")) {
      ...
      if (!isOwner) {
        return {
          isValid: false,
          message:
            "Cannot verify address ownership. Only use addresses you control.",
        };
      }
    }
  } catch (err) {
    return {
      isValid: false,
      message:
        "Must be a valid Stacks address or contract (e.g. ST... or ST....contract-name)",
    };
  }
...
export function verifyAddressOwnership(address: string): boolean {
  try {
...
export function validateLogo(logo: string) {
  if (logo && logo.trim() !== "") {
...
export function validateTwitter(handle: string) {
...
export function validateDiscord(serverId: string) {
...
export function validateMarketType(template: StoredOpinionPoll) {
...
export function isContiguous(data: Array<ScalarMarketDataItem>): boolean {
...
export function validateOnInteraction(
  field: string,
  value: string,
