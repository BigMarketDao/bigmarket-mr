...
| `input` | Input **border** (shadcn convention; consumed by `bm-ui` as `border-input`) | **Next wave** — locked in [`../tokens-and-rules.lock.md`](../tokens-and-rules.lock.md) §1.1 |
| ~~`input-border`~~ | ~~Input border~~ | **Dropped** — replaced by `input` (single token; matches shadcn + existing `bm-ui` consumers) |
...
1. In `tokens.ts` under `colors` add **only** what the first pass actually uses (e.g. `card`, `popover`, `input` [border], a starter `outcome-1..4`).
