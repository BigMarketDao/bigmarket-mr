...
|---|---|---|
| Process, rules, naming contract, migration order, dark-mode requirement | `docs/design/styling-contract.md` | Canonical per `docs/design/README.md` §Single-source rule. |
| Semantic token inventory (which roles exist, quad pattern, domain tokens) | `docs/design/bm-semantic-tokens-proposal.md` | Same. |
| Visual baseline (palette, contrast, fonts, radius base, text-scaling) | `docs/design/skeleton-theme-nouveau.md` + `packages/bm-design/src/vendor/skeleton-nouveau.css` | Doc + vendored primitives are the visual ground truth. |
| Architecture boundaries only (`bm-ui` vs app) | `docs/design/ui-system.md` | Explicitly “layer boundaries only”; defers tokens to design docs. |
...
| Reference UI audit (Polymarket) | `docs/design/polymarket-ui-audit.md` | Supporting, **not** canonical for tokens. Method only — summary tables in repo are unfilled today. |
...
1. `docs/design/sources-of-truth.md` — this report.
2. `docs/design/README.md` — entry point + single-source rule.
3. `docs/design/PROMPTS.md` — executable refactor checklist.
4. `docs/design/styling-contract.md` — English version of the contract, with Skeleton-secondary ↔ accent warning at the top.
5. `docs/design/bm-semantic-tokens-proposal.md` — implemented vs next-wave annotated.
6. `docs/design/skeleton-theme-nouveau.md` — English version.
7. `docs/design/polymarket-ui-audit.md` — English method.
8. `docs/design/ui-system.md` — architecture boundaries.
9. `docs/design/README.md` — design subpack index.

**Suggested commit message (Conventional Commits, English):**

```text
docs(design): introduce docs/design canonical pack for phantom-wallet refactor
...
- Keep legacy docs/design/ in place as history; docs/design is canonical going forward.
```
