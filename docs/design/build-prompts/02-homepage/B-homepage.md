...
> **If Build Prompt B0 already ran:** Task 5 (chrome sweep) is already done. Skip Task 5 and proceed to Tasks 1–4 only. The `card.svelte` and `button-variants.ts` fixes in Task 2 are also already landed — skip those and skip the duplicate bm-ui file entries from the allowed list.
...
1. `docs/design/tokens-and-rules.lock.md` — values + numeric rules + motion + a11y.
2. `docs/design/current-vs-target.md` §3 Gap 1 (market cards), §3 Gap 3 (numbers), §3 Gap 6 (live data).
3. `docs/design/design-philosophy.md` §4 (professional but approachable table), §6 (do/don't).
4. `docs/design/polymarket-analysis.md` §2.1 (app shell), §2.2 (hero card variants).
 ...
 * Lock: docs/design/tokens-and-rules.lock.md §2.
 */
