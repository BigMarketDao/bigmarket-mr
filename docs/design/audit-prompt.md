# Audit prompt — does `docs/design/` give us enough to build an amazing prediction-market UI?

Paste the block below into a senior agent (Claude / Cursor / o1-class). It is a **read-only audit** that compares what `docs/design/` already covers against what is actually required to deliver a visibly more professional, pixel-perfect prediction-market product. The output is a **gap report** — not a refactor.

Do **not** modify `docs/design/` while running this audit. Capture the gaps, then decide separately whether each one becomes a new doc, a new section, a token addition, or a deferred decision.
...
3. The agent will produce a structured Markdown report. Save it as `docs/design/audit-report.md` (or as a PR comment).
4. Review the gaps; only then decide if/how each one lands in `docs/design/`.
...
## Your job
Audit the `docs/design/` pack in this repository. Decide whether it gives an AI agent and a small team enough information to deliver a visibly more professional, pixel-perfect prediction-market UI on the level of Polymarket / Kalshi — not just "better tokens".
...
2. `docs/design/sources-of-truth.md`
3. `docs/design/README.md`
4. `docs/design/styling-contract.md`
5. `docs/design/bm-semantic-tokens-proposal.md`
6. `docs/design/skeleton-theme-nouveau.md`
7. `docs/design/ui-system.md`
8. `docs/design/PROMPTS.md`
9. `docs/design/polymarket-ui-audit.md`
...
Your audit must measure docs/design against that bar, not against a generic "design system" bar.
...
- **Missing** — not in docs/design at all (and not deferrable to a later wave without risking the visual goal).
...
- Is there a populated Polymarket / Kalshi audit anywhere in the repo, or is it still the empty template in `docs/design/polymarket-ui-audit.md`?
...
```
# docs/design gap report

## 1. Summary verdict
One paragraph: can a careful agent ship a Polymarket-grade UI with docs/design alone? If no, what are the top three blockers?
...
Concrete, scoped edits to docs/design that materially raise the audit score. Each item names the file and what to add.

## 7. Implementation drift
Cited file+lines where the codebase diverges from docs/design today, with no action — just visibility.

## 8. Recommended next docs
A list of new doc filenames that, if added under docs/design/, would close the most gaps. For each, one sentence on contents.
```
...
- Prefer **measurable** verdicts ("`docs/design/bm-semantic-tokens-proposal.md` lines 71-79: `success-*` proposed but not in `theme.css`") over generic design advice.
- Stay aligned with the canonical sources of truth in `docs/design/sources-of-truth.md` §A. Do not propose anything that contradicts the behavior-preservation guardrails in `docs/design/PROMPTS.md`.
- Aim for a report that lets a follow-up agent decide, in one pass, exactly which new docs or sections to add to `docs/design/` — without re-reading the codebase.
```
...
- Save the agent's report as `docs/design/audit-report.md`.
