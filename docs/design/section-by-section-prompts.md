...
Allowed:
- New Markdown docs under docs/design/.
- Edits to existing docs under docs/design/ that align with the single-source rule in docs/design/README.md.
...
- Editing docs/design/ legacy files. docs/design/ is canonical.
- Reintroducing DaisyUI utilities.

Conflict rule:
- If two docs disagree, follow the resolution rule in docs/design/README.md "Single-source rule".
...
- When citing Polymarket, cite docs/design/polymarket-research/extracted/<file>.txt:<line> or the screenshot filename in docs/design/assets/polymarket/.
...
- [`docs/design/bm-semantic-tokens-proposal.md`](./design/bm-semantic-tokens-proposal.md) §"Prediction domain" + §"Brand / community"
- [`docs/design/styling-contract.md`](./design/styling-contract.md) §4 "Naming contract" (the `destructive ≠ price-down` rule)
- [`docs/design/skeleton-theme-nouveau.md`](./design/skeleton-theme-nouveau.md) (so values land in the right palette family)
...
- [`docs/design/polymarket-analysis.md`](./design/polymarket-analysis.md) §1.3 + §2.7 (for cross-check values; do NOT copy)

**Output:** `docs/design/tokens-domain.md`, ~150 lines.
...
- [`docs/design/current-vs-target.md`](./current-vs-target.md) §3 Gap 3 ("Numbers look like a different app on every screen")
- [`docs/design/design-philosophy.md`](./design/design-philosophy.md) §6 "Do/Don't"
- [`docs/design/polymarket-analysis.md`](./design/polymarket-analysis.md) §2.7 (numeric formatting comparison)
- [`packages/bm-utilities/src/lib/format.ts`](../packages/bm-utilities/src/lib/format.ts) — existing helpers (read fully; ≤100 lines)

**Output:**

1. New doc `docs/design/formatting.md`, ~120 lines.
...
3. Add `"tabular-nums"` rule to `docs/design/styling-contract.md` §5 "Recipes" (one sentence).
...
- [`docs/design/polymarket-analysis.md`](./design/polymarket-analysis.md) §1.7 (Polymarket's named animations)
- [`packages/bm-design/src/theme.css`](../packages/bm-design/src/theme.css) — read only the `--ease-*` and `--animate-*` lines (search for `--ease`)
- [`docs/design/design-philosophy.md`](./design/design-philosophy.md) §6 "Don't" (decorative animations forbidden)
...
**Output:** `docs/design/motion.md`, ~80 lines.
...
- [`docs/design/audit-report.md`](./audit-report.md) §D (Interaction and accessibility)
...
**Output:** `docs/design/a11y.md`, ~80 lines.
...
**Output:** `docs/design/dark-mode.md`, ~50 lines.
...
- [`docs/design/skeleton-theme-nouveau.md`](./design/skeleton-theme-nouveau.md) §"Charts" (ApexCharts is the library)
- [`docs/design/bm-semantic-tokens-proposal.md`](./design/bm-semantic-tokens-proposal.md) §"Chart structure" + §"Outcome series"
...
- [`docs/design/polymarket-analysis.md`](./design/polymarket-analysis.md) §2.2 (multi-line categorical chart, single-line binary chart, live-tick scalar chart)

**Output:** `docs/design/charts.md`, ~120 lines.
...
- [`docs/design/design-philosophy.md`](./design/design-philosophy.md) §5 "App shell" personality
- [`docs/design/polymarket-analysis.md`](./design/polymarket-analysis.md) §2.1 (Polymarket shell pattern)
...
**Output:** `docs/design/components/shell.md`, ~120 lines.
...
- [`docs/design/design-philosophy.md`](./design/design-philosophy.md) §5 (registers per surface)
- [`docs/design/polymarket-analysis.md`](./design/polymarket-analysis.md) §2.2 (hero variants)
- [`docs/design/tokens-domain.md`](./design/tokens-domain.md) (must be done by Prompt 1)
- [`docs/design/formatting.md`](./design/formatting.md) (must be done by Prompt 2)
...
**Output:** `docs/design/components/market-card.md`, ~250 lines.
...
**Output:** `docs/design/components/market-detail.md`, ~180 lines.
...
**Output:** `docs/design/components/trade-widget.md`, ~250 lines.
...
**Output:** `docs/design/components/confirmation.md`, ~120 lines.
...
**Output:** `docs/design/components/order-book.md`, ~120 lines.
...
**Output:** `docs/design/components/forms.md`, ~120 lines.
...
**Output:** `docs/design/components/modal.md`, ~80 lines.
...
**Output:** `docs/design/components/wallet-modal.md`, ~150 lines.
...
**Output:** `docs/design/components/notifications.md`, ~80 lines. Decide on a library (`svelte-sonner` or hand-rolled), document toast anatomy, durations, stack behavior, dismiss, screen-reader behavior (`aria-live="polite"` for info, `assertive` for errors).
...
**Output:** `docs/design/components/positions.md`, ~120 lines. Position card, positions table, P&L coloring via `price-up` / `price-down`, empty state.
...
**Output:** `docs/design/components/address-pill.md`, ~50 lines + a new `<AddressPill />` primitive in `bm-ui`.
...
**Output:** `docs/design/components/empty-states.md`, ~80 lines. Top-level (illustrated) vs inline (small muted text) patterns.
...
**Output:** `docs/design/components/error-states.md` + new `apps/frontend-c1/src/routes/+error.svelte`, ~100 lines.
...
**Output:** `docs/design/components/skeletons.md`, ~50 lines + `packages/bm-ui/src/lib/components/ui/skeleton/skeleton.svelte`.
...
**Output:** `docs/design/process/lint-and-ci.md` + minimal additions to ESLint config in `apps/frontend-c1` and the workspace packages.
...
**Output:** A single doc `docs/design/process/wave-2-backlog.md` (~80 lines) that lists every token, every component sweep, and every helper that is still outstanding after prompts 1–22, sized S/M/L, with cross-references to the doc that owns the spec.
...
Re-run the audit (`docs/design/audit-prompt.md`) and compare. The expected outcome:
