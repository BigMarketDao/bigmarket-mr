<script lang="ts">
    ...
    import { daoOverviewStore, getStxAddress, isLoggedIn, selectedCurrency } from '@bigmarket/bm-common';
    import {
    ...
    import { aiMarket, allowedTokenStore, exchangeRatesStore, chainStore } from '@bigmarket/bm-common';
	...
	import { appConfigStore, daoConfigStore, requireAppConfig, requireDaoConfig } from '@bigmarket/bm-common';
  ...
  const appConfig = $derived(requireAppConfig($appConfigStore));
    ...
    let suggestion: string = $state(`make a market that asks if the  price of bitcoin will be above or below  112,221.81 USD in 2 hours from now at 16:30 on Sept 29th 2025`);
    ...
    const handlePromptMarket = async (market: {
      ...
      if (
        ...
        try {
          ...
          if (llmMarket.outcomes.length === 2) {
              ...
              >
                <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                  ...
                  <p class="mt-1 flex justify-between text-sm text-gray-500 dark:text-gray-400">
                    <span
                      >Bitcoin Height: {fmtNumber($chainStore.stacks.burn_block_height)}</span
                    >
                    <span
                      >Bitcoin Price: {$exchangeRatesStore[0].currency}
                      {fmtNumber($exchangeRatesStore[0].fifteen)}</span
                    >
                  </p>
                </div>
            ...
            <div class="order-2 space-y-6 lg:order-2 lg:col-span-2">
              ...
              >
                ...
                <div class="py-3 flex justify-between">
                  Duration: <input
                    class="bg-gray-200 text-gray-900 grow mx-5 p-1"
                    type="number"
                    bind:value={newMarketDuration}
                  />
                  End at: {estimateBitcoinBlockTime(
                ...
                <div class="grid grid-cols-5 gap-3">
                  <button
                    ...
                    on:click|preventDefault={() => quickMarket(BTCUSD)}
                  >
                    BTC
                  </button>
                  <button
                    class="inline-flex transform cursor-pointer items-center justify-center rounded-lg bg-gradient-to-r from-orange-500 to-red-600 px-4 py-3 text-sm font-semibold text-white shadow-md transition-all hover:scale-105 hover:from-orange-600 hover:to-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                    on:click|preventDefault={() => quickMarket(STXUSD)}
                  >
                    STX
                  </button>
                  <button
                    class="inline-flex transform cursor-pointer items-center justify-center rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-3 text-sm font-semibold text-white shadow-md transition-all hover:scale-105 hover:from-orange-600 hover:to-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                    on:click|preventDefault={() => quickMarket(ETHUSD)}
                  >
                    ETH
                  </button>
                  <button
                    class="inline-flex transform cursor-pointer items-center justify-center rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-3 text-sm font-semibold text-white shadow-md transition-all hover:scale-105 hover:from-orange-600 hover:to-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                    on:click|preventDefault={() => quickMarket(SUIUSD)}
                  >
                    SUI
                  </button>
                  <button
                    class="inline-flex transform cursor-pointer items-center justify-center rounded-lg bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-3 text-sm font-semibold text-white shadow-md transition-all hover:scale-105 hover:from-orange-600 hover:to-orange-700 focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
                    on:click|preventDefault={() => quickMarket(TONUSD)}
                  >
                    TON
                  </button>
                </div>
                ...
                <div class="mb-6 flex items-center justify-center sm:justify-start">
                  <div class="rounded-lg bg-gray-100 p-1 dark:bg-gray-800">
                    <button
                      ...
                      on:click|preventDefault={() => (showAIURL = !showAIURL)}
                    >
                      From URL
                    </button>
                    <button
                ...
                <div class="grid grid-cols-3 gap-4">
                  <div class="text-center">
                    ...
                    <p class="text-lg font-semibold text-gray-900 dark:text-white">
                      {fmtMicroToStx(
                        $daoOverviewStore.contractData?.marketInitialLiquidity || 0,
                      )}
                    </p>
