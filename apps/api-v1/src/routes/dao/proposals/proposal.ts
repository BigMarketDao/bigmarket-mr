...
export async function getProposalData(votingContract: string, principle: string): Promise<ProposalData> {
	try {
		//console.log('==============================================================')
		const functionArgs = [`0x${serializeCV(contractPrincipalCV(principle.split('.')[0], principle.split('.')[1]))}`];
		const data = {
...
export async function getProposalContractSource(principle: string): Promise<ProposalContract> {
	try {
		...
		console.log('getProposalContractSource: proposalData1: ' + principle + ' -> ' + err.message);
...
export async function fetchAllProposals(): Promise<Array<VotingEventProposeProposal>> {
