...
declare function wallet_unlock(address: string, passphrase?: string): Promise<void>;
...
const DEPLOYERS: Record<Network, string> = {
  ...
  mainnet: 'SP1SCD8ERMTFYE6CK9S0MHWQCP6SY4NAVFJ538A27',
};
...
const CLARITY_ERRORS: Record<string, string> = {
  ...
  u2001: 'not authorized (BANG balance insufficient or gate contract misconfigured)',
};
...
function getNetwork(): Network {
  ...
  if (n !== 'testnet' && n !== 'mainnet') {
    throw new Error(`BIGMARKET_NETWORK must be 'testnet' or 'mainnet' (got: ${n})`);
  }
  return n;
}
...
function bangContract(flag?: string): string {
  ...
  if (!v) throw new Error('BIGMARKET_BANG_CONTRACT not set (or pass --bang-contract).');
  return v;
}
...
function bangClaimContract(flag?: string): string {
  ...
  if (!v) {
    throw new Error('BIGMARKET_BANG_CLAIM_CONTRACT not set (or pass --bang-claim-contract).');
  }
  return v;
}
...
function requireConfirm(flags: Record<string, string | boolean>): void {
  ...
  throw new Error('This is a write action. Re-run with --confirm to broadcast.');
}
...
async function activeWallet(): Promise<string> {
  ...
  if (!wallets.length) throw new Error('No connected wallets — run wallet_list / wallet_unlock first.');
  const w = wallets[0];
...
async function resolveToken(
  ...
  if (tokenFlag.toLowerCase() === 'bang') {
    ...
    const allowed = await call_read_only_function(bme024(net), 'is-allowed-token', [bang]);
    if (!isTrueOk(allowed)) {
      throw new Error(`BANG (${bang}) is not an allowed token for market ${marketId}.`);
    }
    return bang;
  }
  return tokenFlag;
...
async function assertBangGateEligible(wallet: string, gateContract: string): Promise<{
  threshold: bigint;
  decimals: number;
  balance: bigint;
}> {
  const exempt = await call_read_only_function(gateContract, 'get-exempt', [wallet]);
  const bang = bangContract();
  const decimals = Number(toBigInt(await call_read_only_function(bang, 'get-decimals', [])));
  const balance = toBigInt(await call_read_only_function(bang, 'get-balance', [wallet]));

  if (isTrueOk(exempt) || unwrap(exempt) === true) {
    return { threshold: 0n, decimals, balance };
  }
  const thresholdRaw = toBigInt(await call_read_only_function(gateContract, 'get-bang-threshold', []));
  if (thresholdRaw === 0n) {
    return { threshold: 0n, decimals, balance };
  }
...
interface ApiMarketResponse {
  marketId: number;
  unhashedData?: { name?: string; outcomes?: string[]; forumMessageId?: string };
  marketData?: {
    categories?: string[];
    stakes?: number[];
...
function mapApiMarket(raw: ApiMarketResponse): MarketSummary {
  ...
  for (let i = 0; i < categories.length; i++) {
    impliedProbability[categories[i]] = total > 0 ? (stakes[i] ?? 0) / total : 0;
  }
...
async function listMarkets(): Promise<void> {
  ...
  for (let id = 1; id < 10_000; id++) {
    const raw = await getJson<ApiMarketResponse>(`${base}/pm/markets/${id}/categorical`);
  ...
  for (const m of out) {
    ...
    for (const cat of m.categories) {
      ...
      console.log(`    ${cat.padEnd(16)} pool=${pool}  p=${p !== undefined ? (p * 100).toFixed(2) + '%' : '—'}`);
    }
  }
}
...
async function readMarket(args: ParsedArgs): Promise<void> {
  ...
  const data = await call_read_only_function(bme024(net), 'get-market-data', [id]);
  const accounting = await call_read_only_function(bme024(net), 'get-market-accounting', [id]);
  let stakes: unknown = null;
  if (wallets[0]) {
    stakes = await call_read_only_function(bme024(net), 'get-stake-balances', [id, wallets[0]]);
  }
  console.log(JSON.stringify({ id, data: unwrap(data), accounting: unwrap(accounting), stakes: unwrap(stakes) }, bigintReplacer, 2));
}
...
async function predict(args: ParsedArgs): Promise<void> {
  ...
  if (!Number.isFinite(marketId) || !category || !amountStr) {
    throw new Error('Usage: predict <market-id> <category> <amount-ustx> [--token bang]');
  }
  const wallet = await activeWallet();
  const token = await resolveToken(net, marketId, args.flags.token as string | undefined);
  ...
  const cost = toBigInt(
    await call_read_only_function(bme024(net), 'get-share-cost', [marketId, indexOfCategory(category), estShares]),
  );
  ...
  const { txid } = await wrapClarityCall(() =>
    call_contract(bme024(net), 'predict-category', [marketId, minShares, category, token, maxCost]),
  );
  ...
  recordHistory({
    ...
    wallet,
    txid,
    params: { marketId, category, amount: amount.toString(), token, minShares: minShares.toString(), maxCost: maxCost.toString() },
  });
}
...
async function sell(args: ParsedArgs): Promise<void> {
  ...
  if (!Number.isFinite(marketId) || !category || !sharesStr) {
    throw new Error('Usage: sell <market-id> <category> <shares> [--token bang]');
  }
  const wallet = await activeWallet();
  const token = await resolveToken(net, marketId, args.flags.token as string | undefined);

  const balances = await call_read_only_function(bme024(net), 'get-token-balances', [marketId, wallet]);
  if (!balances) throw new Error('No position found in this market.');

  const grossRefund = toBigInt(
    await call_read_only_function(bme024(net), 'get-share-cost', [marketId, 0, shares]),
  );
  ...
  const { txid } = await wrapClarityCall(() =>
    call_contract(bme024(net), 'sell-category', [marketId, minRefund, category, token, shares]),
  );
  console.log(`txid: ${txid}`);
  recordHistory({
    ts: new Date().toISOString(),
    ...
    wallet,
    txid,
    params: { marketId, category, shares: shares.toString(), token, minRefund: minRefund.toString() },
  });
}
...
async function addLiquidity(args: ParsedArgs): Promise<void> {
  ...
  if (!Number.isFinite(marketId) || !amountStr) {
    throw new Error('Usage: add-liquidity <market-id> <amount-ustx> [--token bang]');
  }
  const wallet = await activeWallet();
  const token = await resolveToken(net, marketId, args.flags.token as string | undefined);

  const accounting = (await call_read_only_function(bme024(net), 'get-market-accounting', [marketId])) as Record<string, unknown>;
  const expectedTotal = toBigInt((unwrap(accounting) as Record<string, unknown>)?.['total-stakes'] ?? 0n);
  ...
  const { txid } = await wrapClarityCall(() =>
    call_contract(bme024(net), 'add-liquidity', [marketId, amount, expectedTotal, 500, token]),
  );
  console.log(`txid: ${txid}`);
  recordHistory({
    ts: new Date().toISOString(),
    ...
    wallet,
    txid,
    params: { marketId, amount: amount.toString(), token, expectedTotal: expectedTotal.toString() },
  });
}
...
async function removeLiquidity(args: ParsedArgs): Promise<void> {
  ...
  if (!Number.isFinite(marketId) || !sharesStr) {
    throw new Error('Usage: remove-liquidity <market-id> <lp-shares> [--token bang]');
  }
  const wallet = await activeWallet();
  const token = await resolveToken(net, marketId, args.flags.token as string | undefined);

  const accounting = await call_read_only_function(bme024(net), 'get-market-accounting', [marketId]);
  console.log('Preview — remove-liquidity');
  console.log(`  market:    ${marketId}`);
  console.log(`  lp shares: ${shares}`);
  console.log(`  pool:      ${JSON.stringify(unwrap(accounting), bigintReplacer)}`);
  const minRefund = applySlippage(shares);
  console.log(`  min refund: ${minRefund}`);
  requireConfirm(args.flags);

  const { txid } = await wrapClarityCall(() =>
    call_contract(bme024(net), 'remove-liquidity', [marketId, shares, minRefund, token]),
  );
  console.log(`txid: ${txid}`);
  recordHistory({
    ts: new Date().toISOString(),
    ...
    wallet,
    txid,
    params: { marketId, shares: shares.toString(), token, minRefund: minRefund.toString() },
  });
}
...
async function claimRewards(args: ParsedArgs): Promise<void> {
  const net = getNetwork();
  const wallet = await activeWallet();
  const before = toBigInt(await call_read_only_function(bme030(net), 'get-balance', [1, wallet]));
  const epoch = toBigInt(await call_read_only_function(bme030(net), 'get-epoch', []));
  ...
  console.log(`txid: ${txid}`);
  const after = toBigInt(await call_read_only_function(bme030(net), 'get-balance', [1, wallet]));
  ...
  recordHistory({
    ...
    wallet,
    txid,
    params: { before: before.toString(), after: after.toString(), epoch: epoch.toString() },
  });
}
...
async function status(): Promise<void> {
  ...
  for (const t of BIGR_TIERS) {
    const b = toBigInt(await call_read_only_function(bme030(net), 'get-balance', [t, wallet]));
    tiers[t] = b.toString();
    if (b > 0n) currentTier = Math.max(currentTier, t);
  }
  const epoch = toBigInt(await call_read_only_function(bme030(net), 'get-epoch', []));
  const epochDuration = toBigInt(await call_read_only_function(bme030(net), 'get-epoch-duration', []));
  ...
  if (!bangFeaturesEnabled()) {
    console.log('BANG / create-market:  disabled (set BIGMARKET_BANG_ENABLED=true when token is live)');
    return;
  }

  const bang = bangContract();
  const decimals = Number(toBigInt(await call_read_only_function(bang, 'get-decimals', [])));
  const bangBal = toBigInt(await call_read_only_function(bang, 'get-balance', [wallet]));
  ...
  console.log(`create-market gate:  ${eligible ? 'ELIGIBLE (≥100 BANG)' : 'INELIGIBLE (<100 BANG)'}`);
}
...
async function createMarket(args: ParsedArgs): Promise<void> {
  ...
  if (pos.length < 4) {
    throw new Error('Usage: create-market "<title>" <cat1> <cat2> [catN…] <seed-ustx>');
  }
  const title = pos[0];
  ...
  const gate = bangGateContract(args.flags['bang-gate-contract'] as string | undefined);
  const wallet = await activeWallet();
  ...
  const meta = { title, categories, createdBy: wallet, timestamp: new Date().toISOString() };
  ...
  console.log(`  BANG balance:    ${formatToken(balance, decimals)}  (100 will be burned)`);
  console.log(`  after creation:  ${formatToken(balance - 100n * 10n ** BigInt(decimals), decimals)}`);
  ...
  const token = await resolveToken(net, 0, args.flags.token as string | undefined);
  ...
  const marketId = result !== undefined ? toBigInt(result).toString() : 'unknown';
  console.log(`txid:      ${txid}`);
...
async function claimAirdrop(args: ParsedArgs): Promise<void> {
  ...
  const net = getNetwork();
  const claim = bangClaimContract(args.flags['bang-claim-contract'] as string | undefined);
  const wallets = await wallet_list();
  if (!wallets.length) throw new Error('No connected wallets.');
  const wallet = wallets[0];

  const elig = (await call_read_only_function(claim, 'get-eligibility', [wallet])) as Record<string, unknown>;
  ...
  if (e.claimed === true || (e as { claimed?: unknown }).claimed === 1) {
    console.log(`Already claimed: ${e.amount ?? '?'} BANG at ${e['claim-date'] ?? e.claimDate ?? '?'}`);
    return;
  }
  ...
  const { txid } = await wrapClarityCall(() => call_contract(claim, 'claim-airdrop', [tier]));
  console.log(`txid: ${txid}`);
  recordHistory({
    ts: new Date().toISOString(),
...
function findMessageLevel(node: ForumThreadNode, targetId: string): number | undefined {
...
async function comment(args: ParsedArgs): Promise<void> {
  ...
  const marketId = Number(marketIdStr);
  const content = (args.flags.content as string | undefined) ?? contentParts.join(' ');
  ...
  if (title && !/^[\x20-\x7E]*$/.test(title)) throw new Error('--title must contain only printable ASCII characters.');
  if (!/^[\x20-\x7E]*$/.test(content)) throw new Error('content must contain only printable ASCII characters.');
  ...
  const raw = await getJson<ApiMarketResponse>(`${API_BASES[net]}/pm/markets/${marketId}/categorical`);
  ...
  const thread = await getJson<ForumThreadNode>(`${forumApi(net)}/forum/messages/${forumMessageId}`);
  if (!thread) {
    throw new Error(`Forum thread ${forumMessageId} not found on ${net} forum API.`);
  }
  ...
  if (!replyTo || replyTo === forumMessageId) {
    ...
    if (parentLevel === undefined) {
      ...
      console.warn(`  ⚠ parent message ${replyTo} not found in thread tree — defaulting level to 3.`);
      level = 3;
    } else {
      level = parentLevel + 1;
    }
  ...
  if (!publicKey) {
    const { verification } = await sip018_hash({ message: messageToSign, domain });
    const recovered = await sip018_verify({ messageHash: verification, signature });
  ...
  console.log(`  message-id: ${messageId}`);
  console.log(`  level:      ${level}`);
  console.log(`  thread:     ${forumApi(net)}/forum/messages/${forumMessageId}`);
  ...
  recordHistory({
    ...
    wallet,
    txid: messageId,
    params: { marketId, forumMessageId, parentId, level, title, content, messageId },
  });
}
